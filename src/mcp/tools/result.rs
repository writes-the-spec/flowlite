...
/// agent that got back `scheduled` with no warning would poll a status that cannot change.
///
...
/// "scheduled"}` with nothing saying it will never move, which is the exact failure the
...
/// legitimate; an agent that got back `scheduled` with no warning would poll a status that
