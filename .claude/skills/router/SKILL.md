...
- **`serve`'s pool attaches `mem`, so one pool reaches both databases.** Jobs and schedules are `mem` tables and are still queried through `conn_pool` - a route never calls `with_fresh_mem`, which is the MCP server's path. `AppState.memory_conn` is held and never read on purpose: dropping it would drop the shared-cache in-memory database out from under the pool, leaving it attaching an empty one. It carries an `#[allow(dead_code)]` saying so, which is why the build is warning-free — a new dead-code warning means something really is unused.
...
- **Assets are embedded, not served from disk.** [assets.rs](../../../src/router/app/assets.rs) uses `rust_embed` over `assets/`, so a release binary carries its own CSS and vendored htmx.
