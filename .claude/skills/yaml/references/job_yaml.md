...
`CRUD::init` writes each task's `depends_on` to **two** places from the same list: `task.depends_on` as a JSON array on the task row, and one normalized `task_dependent` row per edge. `task.depends_on` is the one the runtime resolves (`TaskRunDispatcher::get_dependent_task_runs`); `task_dependent` is currently read by nothing. See the [task skill](../../task/SKILL.md).

Submitting the job then creates one `task_run` per task, all `Pending` — dependency order is enforced at dispatch time, not at submission. See the [orchestrator skill](../../orchestrator/references/task_run.md).
