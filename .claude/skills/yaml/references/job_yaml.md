...
| `keep_runs` | no | `[job_defaults]`, `100` | How many of this job's newest finished runs retention keeps. **`0` keeps every run of this job**, leaving `[retention] keep_runs_total` as the only bound. Not yet read by anything — carried onto `mem.job` ahead of the retention service that will read it. |
