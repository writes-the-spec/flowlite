...
| [mcp/tools/result.rs](../../../src/mcp/tools/result.rs) | a submitted run comes back `submitted`, and against an unserved directory it will never reach `queued` at all, so the result carries a warning |
