...
- `CRUD::submit_job` — copies `command`, `depends_on`, `timeout`, `max_retries`, `retry_delay`, `env` and `working_dir` onto every `task_run` it inserts. This is the snapshot the orchestrator then runs on.
