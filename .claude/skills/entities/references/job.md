...
- `CRUD::submit_job` ([src/crud/multistatements/misc.rs](../../../../src/crud/multistatements/misc.rs)) — copies `name` and `description` onto the `job_run` it inserts as `job_name`/`job_description`, and bails with `Job '<id>' not found` if the row is missing. It also passes `parameters` to `resolve_job_parameters`, which raises if a caller's override names a parameter this row does not declare, and merges both `env` under each task's own `env:` and `secret_env` under each task's own `secret_env:`, via the same generalized `merge_job_and_task_maps`, called once per block, the task's own winning a name both declare within that block. That is not the whole story, because the two merges are not independent: after both run, a task's own declaration in either block evicts the job's contribution to the *other* block for that same name — a name the task declares in `secret_env:` is dropped from the merged `env`, and a name it declares in `env:` is dropped from the merged `secret_env`. The stored `task_run` row this produces therefore never has one name in both `env` and `secret_env` — a task can override a job's plain variable with a secret, or a job's secret with a plain variable, but never end up carrying both.
