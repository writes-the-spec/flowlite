...
- `max_parallel_runs` is enforced in exactly one place, `JobRunDispatcher::settle_as_queued`. Nothing rejects a submission for being over it — the run is created `Submitted` like any other, and once `JobRunReleaser` moves it to `Queued` it queues there instead of starting. See the [orchestrator skill](../../orchestrator/references/job_run.md).
