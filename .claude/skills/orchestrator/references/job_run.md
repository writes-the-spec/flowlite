...
- **Nothing here is covered by a test.** Every guard is inlined in its own outcome, so there is no pure function left to reach, and both ordering rules above live purely in the call order in `handle_running_job_run`. Reordering steps 3–5 compiles, passes the suite, and silently changes what a stopped or mixed-outcome run reports. Steps 1 and 2 are position-independent — step 1 because it is exclusive with everything, step 2 because those three re-ask `all_finished` themselves — so the precedence among 3–5 is the whole of what the call order carries here.
