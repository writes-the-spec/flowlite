...
3. **neither?** → left `Scheduled`, writing nothing. `derive_next_status` returns `Scheduled` and the match arm answers it with `Ok(())`, so doing nothing is a named outcome rather than an unhandled branch. Unlike five of the other chains this one ends there rather than in a `settle_unclaimed` that writes `Invalid`: a run that is simply not due yet is this service's common case, not the symptom of a missing rung. Any *other* status, and any error, does write `Invalid`.
