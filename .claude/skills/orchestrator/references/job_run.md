...
4. **any other status, or an error** → `set_to_invalid`, with a log line saying it is a bug. Unreachable, `is_stopped` and `is_due` covering every case between them, so it exists for the day a status is added and this match is not.
...
4. **any other status, or an error** → `set_to_invalid`, logged as a bug. Unreachable while those two checks cover every case.
...
7. none of them → an `Err`, which `set_to_invalid` settles the row on. Unreachable while 1–6 cover every status.
