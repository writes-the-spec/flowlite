...
4. `None`, or an error → `set_to_invalid`, logged as a bug. Unreachable while the three checks cover every dependency set.
...
| anything left | `Err` → `set_to_invalid` | `Invalid`, logged as a bug — unreachable while the returns above cover every attempt status |
