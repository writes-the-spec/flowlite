...
**A status nothing claims settles the row `Invalid`**, logged as a bug. None of those arms is reachable while each `derive_next_status` covers every case — they exist for the day a status is added and a match arm is not, which is not hypothetical: it is what both monitors did while `Invalid` itself was being wired in. Leaving such a row where it is would stop the job scheduling and say so only in a log; settling ends the run, tells whoever the job named, and lets the next one through.

**A read that failed is not one of those, and never settles anything.** `Invalid` is terminal, so deciding it on a failed query would make a moment's contention on the database permanent — the run would never recover, and its retries would be gone with it. Every `Err` goes back to `Poller`, which logs it with the row id and comes round again on the next pass. `TaskRunAttemptMonitor` puts its child process back in `TaskRunAttemptChildren` first, being the only service holding one.

**That is why "nothing claims it" is a value rather than an error.** `JobRunMonitor`'s decision reads nothing but its arguments and so returns a plain `Option`; `TaskRunDispatcher`'s and `TaskRunMonitor`'s return `Result<Option<_>>`, where the `None` is the undecidable set and the `Err` is only the query or the retry insert that failed. Collapsing the two into one `Err` is what made a transient failure look like a verdict.
