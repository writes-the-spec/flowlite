...
Together they give every row the same shape: `Queued → Running → <finished>`, or `Queued → Skipped` if it never got going. **Nothing else writes a run status** — no request handler, no CLI command.
