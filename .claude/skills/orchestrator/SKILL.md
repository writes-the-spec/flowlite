...
Telling somebody a run broke is **not** one of them. `JobRunMonitor` leaves a [`job_run_notification`](../entities/references/job_run_notification.md) row open as it finishes a failed run, and the [NotificationService](../notifications/SKILL.md) — started by `serve` alongside the orchestrator, the way the [Scheduler](../scheduler/SKILL.md) is — delivers it. Nothing in the orchestrator calls that service and it calls nothing back. Delivery is slow and sometimes fails for hours, which is exactly the work a monitor must not be holding while it is meant to be finishing everyone else's runs.
...
**The publish rule:** a service calls `self.signals.publish()` after writing a status another service selects on, or after inserting a row another service selects on — `JobRunMonitor`'s notification insert is an example of the second, and it rides the publish its status write was already doing rather than adding one — never after a data-only write. `TaskRunAttemptMonitor::insert_output`, which appends whatever a running attempt's readers delivered on this pass, is the example worth remembering: it deliberately does not publish, because publishing there would wake all six pollers at least once a second, per running attempt, and turn the wake-up bus back into the polling loop it replaced.
