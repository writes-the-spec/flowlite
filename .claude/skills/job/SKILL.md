...
- **`JobRunDispatcher`** ([src/orchestrator/job_run_dispatcher.rs](../../../src/orchestrator/job_run_dispatcher.rs)) — polls once a second for `Pending` job runs. Each one is either skipped (`handle_stopped_job_run`, if a stop signal exists — see below) or started (`handle_start_job_run`, which sets `status = Running` and `started_at`).
- **`JobRunMonitor`** ([src/orchestrator/job_run_monitor.rs](../../../src/orchestrator/job_run_monitor.rs)) — polls once a second for `Running` job runs. For each, once every task run has left `Pending`/`Running`, it decides the job run's terminal status.

Both are started independently by `Orchestrator::start` ([src/orchestrator/orchestrator.rs](../../../src/orchestrator/orchestrator.rs)), which `serve` calls, and both follow the same shape: `new(crud, conn_pool)`, then `start()` to spawn the loop, with a private `run()` doing the per-second polling. Keep them decoupled — new job-run lifecycle stages should be added as another status-driven poller rather than by having one service call into another.
...
A `job_run_stop` row (disk-persisted, [src/crud/job_run_stop.rs](../../../src/crud/job_run_stop.rs)) is an insert-only cancellation signal keyed by `job_run_id` — there's no "cancel" status update on `job_run` directly. `JobRunDispatcher`, `TaskRunDispatcher` and `TaskRunAttemptMonitor` all poll for a matching stop row on every tick and, if present, finish what they own themselves — the job run `Skipped`, task runs that never started `Skipped` too, in-flight ones `Aborted`. When adding new run-control features (pause, retry-all, etc.), follow this same pattern: a small insert-only signal table that the running loops check, rather than mutating run state directly from an unrelated code path.
