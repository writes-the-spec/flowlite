...
- **`JobRunDispatcher`** ([src/orchestrator/job_run_dispatcher.rs](../../../src/orchestrator/job_run_dispatcher.rs)) — polls for `Pending` job runs, oldest first, once a second or as soon as a wake-up arrives, whichever comes first. Each one is either skipped (`transition_to_skipped`, if a stop signal exists — see below) or started (`transition_to_running`, which sets `status = Running` and `started_at`).
