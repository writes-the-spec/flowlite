...
- **`JobRunDispatcher`** ([src/orchestrator/job_run_dispatcher.rs](../../../src/orchestrator/job_run_dispatcher.rs)) — polls for `Pending` job runs once a second or as soon as a wake-up arrives, whichever comes first. Each one is either skipped (`handle_stopped_job_run`, if a stop signal exists — see below) or started (`handle_start_job_run`, which sets `status = Running` and `started_at`).
...
Both are started independently by `Orchestrator::start` ([src/orchestrator/orchestrator.rs](../../../src/orchestrator/orchestrator.rs)), which `serve` calls, and both follow the same shape: `impl Service` ([src/poller.rs](../../../src/poller.rs)) for the type, then a `Poller::new(...).start()` line at the call site. The `Poller` owns the loop, the wake-ups and the error handling; the service holds nothing but `select` and `handle`. Keep them decoupled — new job-run lifecycle stages should be added as another status-driven poller rather than by having one service call into another.
