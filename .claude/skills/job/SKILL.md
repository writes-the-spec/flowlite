...
- **`JobRunDispatcher`** ([src/orchestrator/job_run_dispatcher.rs](../../../src/orchestrator/job_run_dispatcher.rs)) — polls for `Pending` job runs, oldest first, once a second or as soon as a wake-up arrives, whichever comes first. Each one is settled as exactly one of skipped (`settle_as_skipped`, if a stop signal exists — see below), still pending (`settle_as_pending`, if its job is already at `max_parallel_runs`, which writes nothing), or running (`settle_as_running`, which sets `status = Running` and `started_at`); a row that matches none of the three raises an error rather than being left to sit. That gate is the only place the limit is enforced — submitting is never rejected for being over it.
