...
- **A command never imports from `src/mcp/` or `src/router/`,** beyond `serve` and `mcp` starting them - which [tests/frontend_boundaries.rs](../../../tests/frontend_boundaries.rs) allows by name. Anything the CLI shares with another frontend (`format`, `limits`, `JobRunDetail`, `wait_for_job_run`, `parse_job_run_status`) lives in `src/shared/`. See the `frontends` skill.
