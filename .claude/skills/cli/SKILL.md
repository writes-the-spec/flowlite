...
- Data-accessing commands build a `CRUD::new(Arc::new(toolkit))` from the connection and call `crud.init(&mut conn)` before querying, then use the `crud::<entity>` query structs (see the `crud` skill) — don't write raw SQL in CLI code.
