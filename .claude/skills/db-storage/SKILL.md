...
Whichever shape it is, the handle has to come from `Toolkit`. A connection opened any other way has no `mem` attached, so every query that reaches into a config table — `select_job_runs` joins `mem.job` for the job name — fails on the missing schema.
