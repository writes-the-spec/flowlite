...
`CRUD::init` ([src/crud/crud.rs](../../../src/crud/crud.rs)) inserts one `mem.schedule` row plus one `mem.schedule_job` row per listed job, and computes the first `next_run` right there with `CronTrigger::get_next_run(None)`. Both tables are in-memory config data — see [schedule](../entities/references/schedule.md) and [schedule_job](../entities/references/schedule_job.md) in the entities skill.
...
- **`schedule_job.parameters` reaches the command.** `handle_due_schedule` passes it straight to `CRUD::submit_job` as that call's overrides, which checks it against the job's declared `parameters` and raises if a name here isn't one of them — so a typo in a schedule's `parameters` fails that job on every occurrence, the same "logged and skipped" way any other submit failure does (see the loop above).
